...
[x] 1a. Complete `upsert_school()`: write the Python wrapper that takes a `row` dict, builds the parameter tuple, and executes the INSERT...ON CONFLICT SQL you've already drafted. This is the dict→tuple mechanic — the new learning.

[x] 1b. Write `upsert_snapshot()`: same pattern, but the ON CONFLICT target is the composite key `(cds_code, academic_year)`. Handles nullable INTEGER for suppressed demographic cells, REAL for percentages.
...
[x] 2a. Write a CSV reader that opens the CDE file, prints headers on first run (confirm column names), and maps CDE column names to your snake_case schema names.

[x] 2b. Build the per-row type coercion: parse integers (NULL on failure), parse reals for percentages (NULL on failure), pass text through, strip/normalize whitespace.

[x] 2c. Wire the loop: for each row, call `upsert_school()` (identity), then `upsert_snapshot()` (yearly data). Identity first, snapshot second — the FK requires it.

[x] 2d. Run against the real 2025-26 CSV. Verify row counts, spot-check a few schools by CDS code, confirm the composite key prevents duplicates on re-run.
...
[x] 3a. Resolve your home address to lat/lng. One-time call to the free US Census Geocoder or Nominatim. Store in `.env` as `HOME_LAT`/`HOME_LNG`.
...
[x] 3c. Pick a traffic-aware travel-time provider (Google Routes, TravelTime, or Amazon Location). Get an API key, add to `.env`.
...
[x] 4a. Run ~~`npm run login`~~ `capture_login.py` on your machine to capture a live session (`auth_state.json`).

[WIP] 4b. Run ~~`npm run search`~~ `search_edjoin.py` in discovery mode. Save `discovery.html`, inspect it, and update the `SELECTORS` block in `search_edjoin.js` to match edjoin's actual DOM.

4c. Set real criteria in the `CRITERIA` block. Run a real scrape. Inspect `results.csv` for correctness.

4d. Implement FastAPI pathway for Playwright, with early draft embeded HTMX & SQLite pipeline que/history preview

4e. (Later, Phase 5) Link scraped jobs to schools in the database — this requires fuzzy matching between edjoin's school/district text and your CDE-sourced `school_name`/`district_name`. ~~This is the one place an LLM call earns its keep.~~
...
Implement `rapidfuzz` with matching scorer.
